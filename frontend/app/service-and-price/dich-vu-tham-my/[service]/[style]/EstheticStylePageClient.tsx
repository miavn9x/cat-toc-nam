...
export default function EstheticStylePageClient({
  params,
}: {
  params: { service: string; style: string }
}) {
  const serviceData = DataService.getService("esthetic-services", params.service)
  const styleData = DataService.getStyle("esthetic-services", params.service, params.style)
  ...
  const relatedStyles = DataService.getServiceStyles("esthetic-services", params.service).filter(
    (s) => s.id !== params.style,
  )
  const displayedRelatedStyles = showAllStyles ? relatedStyles : relatedStyles.slice(0, 8)
  ...
  const prevModalImage = () => {
    setModalImageIndex((prev) => (prev - 1 + styleData.images.length) % styleData.images.length)
  }
        ...
        <nav className="flex" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm text-gray-400 overflow-x-auto">
            ...
            <li className="flex items-center">
              ...
              <Link
                href={`/service-and-price/dich-vu-tham-my/${params.service}`}
            ...
            <li className="flex items-center">
              ...
              <span className="ml-2 text-emerald-300">{styleData.name}</span>
            </li>
          </ol>
        </nav>
      </div>
        ...
        <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 md:gap-10 lg:gap-12 xl:gap-16 items-start">
            <div className="order-2 lg:order-1 space-y-4 sm:space-y-6 md:space-y-8">
              ...
              <p className="text-gray-300 text-lg mb-8">{styleData.description}</p>

              <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
                <div className="flex items-center justify-between text-sm sm:text-base">
                  <span className="text-gray-500">Độ khó:</span>
                  <span className="text-emerald-300">{styleData.difficulty}</span>
                </div>
                <div className="flex items-center justify-between text-sm sm:text-base">
                  <span className="text-gray-500">Bảo dưỡng:</span>
                  <span className="text-emerald-300">{styleData.maintenance}</span>
                </div>
                {styleData.faceShape && styleData.faceShape.length > 0 && (
                  <div className="text-sm sm:text-base">
                    <span className="text-gray-500">Phù hợp: </span>
                    <span className="text-emerald-300">{styleData.faceShape.join(", ")}</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                <Link
                  href="/booking"
                  className="px-8 py-3 bg-gradient-to-r from-emerald-500 to-green-500 text-white font-semibold rounded-full hover:shadow-lg hover:shadow-emerald-500/25 transition-all text-center"
                >
                  Đặt Lịch Ngay
                </Link>
                <Link
                  href="/lien-he"
                  className="px-8 py-3 border border-emerald-400/50 text-emerald-300 rounded-full hover:bg-emerald-400/10 transition-all text-center"
                >
                  Liên Hệ Tư Vấn
                </Link>
              </div>
            ...
            <div className="order-1 lg:order-2">
              <div className="space-y-4">
                ...
                <div className="relative rounded-2xl overflow-hidden shadow-2xl cursor-pointer group">
                  <img
                    src={
                      styleData.images[selectedImage] || "/placeholder.svg?height=600&width=400" || "/placeholder.svg"
                    }
      ...
      <section className="py-8 sm:py-12 md:py-16 lg:py-20">
        <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          <div className="bg-white/5 backdrop-blur-xl rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 border border-emerald-400/20">
            ...
            <p className="text-sm sm:text-base md:text-lg text-gray-300 mb-6 sm:mb-8">{serviceData.longDescription}</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
              <div>
                <h3 className="text-lg font-medium text-emerald-300 mb-4">Dịch Vụ Bao Gồm</h3>
              ...
              <div>
                <h3 className="text-lg font-medium text-emerald-300 mb-4">Lợi Ích</h3>
                <ul className="space-y-4">
          ...
          <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
            <div className="text-center mb-6 sm:mb-8">
              ...
              <p className="text-sm sm:text-base text-gray-400 max-w-2xl mx-auto mb-4 sm:mb-6 px-4">
                Khám phá các kiểu dáng khác trong dịch vụ {serviceData.title.toLowerCase()}
              </p>
              ...
              {relatedStyles.length > 8 && (
                ...
                >
                  {showAllStyles ? (
                    ...
                    <>
                      Xem tất cả {relatedStyles.length} kiểu dáng
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
              {displayedRelatedStyles.map((s, index) => (
                <Link
                  href={`/service-and-price/dich-vu-tham-my/${params.service}/${s.id}`}
                  key={index}
                  className="group block"
                >
                  <div className="bg-white/5 backdrop-blur-xl rounded-xl overflow-hidden border border-white/10 hover:border-emerald-400/30 transition-all duration-300">
                    <div className="aspect-[3/4] overflow-hidden relative">
                      <img
                        src={s.images[0] || "/placeholder.svg?height=600&width=400"}
                        alt={s.name}
                    ...
                    <div className="p-3 sm:p-4">
                      <h3 className="text-sm sm:text-lg font-medium text-white group-hover:text-emerald-300 transition-colors mb-2 line-clamp-1">
                        {s.name}
                      </h3>
                      <p className="text-gray-400 text-xs sm:text-sm line-clamp-2 mb-3">{s.description}</p>
                      <div className="space-y-1 sm:space-y-2">
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-gray-500">Độ khó:</span>
                          <span className="text-emerald-300">{s.difficulty}</span>
                        </div>
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-gray-500">Bảo dưỡng:</span>
                          <span className="text-emerald-300">{s.maintenance}</span>
                        </div>
                        {s.faceShape && s.faceShape.length > 0 && (
                          <div className="text-xs">
                            <span className="text-gray-500">Phù hợp: </span>
                            <span className="text-emerald-300 line-clamp-1">{s.faceShape.join(", ")}</span>
                          </div>
                        )}
                      </div>
                    </div>
      ...
      <section className="py-16 sm:py-24 bg-gradient-to-br from-black to-slate-900/80">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="bg-white/5 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-8 sm:p-12 border border-white/10">
            <h2 className="text-2xl sm:text-4xl font-light text-white mb-6">Sẵn Sàng Trải Nghiệm {styleData.name}?</h2>
            <p className="text-gray-300 mb-8 max-w-2xl mx-auto">
              Đặt lịch ngay hôm nay để trải nghiệm kiểu dáng {styleData.name} trong dịch vụ{" "}
              {serviceData.title.toLowerCase()} tại Winchair Beauty Spa.
            </p>
                ...
                {styleData.images.map((image, index) => (
                  <button
                    ...
                    className={`w-12 h-12 rounded-md overflow-hidden transition-all ${
                      modalImageIndex === index ? "ring-2 ring-emerald-400" : "opacity-60 hover:opacity-100"
                    }`}
                  >
                    <img
                      src={image || "/placeholder.svg"}
