...
export default function WomenServicePageClient({
  params,
}: {
  ...
  const allStyles = DataService.getServiceStyles("women-hair", params.service);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 md:gap-10 lg:gap-12 xl:gap-16 items-start">
            <div className="order-2 lg:order-1 space-y-4 sm:space-y-6 md:space-y-8">
              ...
              <div className="flex flex-col xs:flex-row sm:flex-row gap-3 sm:gap-4 md:gap-6">
                <Link
                  href="/booking"
                  className="px-6 sm:px-8 py-2.5 sm:py-3 bg-gradient-to-r from-pink-500 to-rose-500 text-white font-semibold rounded-full hover:shadow-lg hover:shadow-pink-300/30 transition-all text-center text-sm sm:text-base transform hover:scale-105"
                >
                  Book Now
                </Link>
                <Link
                  href="/contact"
                  className="px-6 sm:px-8 py-2.5 sm:py-3 border-2 border-pink-300 text-pink-600 rounded-full hover:bg-pink-50 transition-all text-center text-sm sm:text-base"
                >
            ...
            <div className="order-1 lg:order-2">
              <div className="space-y-3 sm:space-y-4">
                {/* Main Image */}
                <div className="relative rounded-xl sm:rounded-2xl overflow-hidden shadow-xl cursor-pointer group border border-pink-100">
                  <img
                    src={
            ...
            <div className="relative">
                    ...
                    <div key={slideIndex} className="w-full flex-shrink-0">
                      <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
                        {displayedPopularStyles
                          ...
                          .map((style, index) => (
                            <Link
                              href={`/service-and-price/womens-hair-services/${serviceData.id}/${style.id}`}
                              key={index}
                              className="group block"
                            >
                              <div className="bg-white rounded-lg sm:rounded-xl overflow-hidden border border-pink-100 hover:border-pink-200 hover:shadow-xl transition-all duration-300">
                                ...
                                <div className="p-2.5 sm:p-3 md:p-4">
                                  <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                                    <h3 className="text-xs sm:text-sm md:text-base lg:text-lg font-medium text-gray-800 group-hover:text-pink-600 transition-colors line-clamp-1">
                                      {style.name}
                                    </h3>
                                    <span className="px-1.5 sm:px-2 py-0.5 sm:py-1 bg-pink-100 text-pink-600 text-xs rounded-full flex-shrink-0">
                                      Popular
                                    </span>
                                  </div>
              ...
              {displayedPopularStyles.length > stylesPerSlide.desktop && (
                <>
                  <button
                    onClick={prevSlide}
                    className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 sm:-translate-x-4 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-white border border-pink-200 rounded-full flex items-center justify-center text-pink-600 hover:bg-pink-50 hover:shadow-lg transition-all duration-300 z-10"
                  ...
                  <button
                    onClick={nextSlide}
                    className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 sm:translate-x-4 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-white border border-pink-200 rounded-full flex items-center justify-center text-pink-600 hover:bg-pink-50 hover:shadow-lg transition-all duration-300 z-10"
      ...
      {allStyles.length > 0 && (
        <section
          id="all-styles"
          className="py-8 sm:py-12 md:py-16 lg:py-20 bg-gradient-to-b from-pink-50 to-white"
        >
          <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
            <div className="text-center mb-6 sm:mb-8">
              ...
              <p className="text-sm sm:text-base text-gray-600 max-w-2xl mx-auto mb-6">
                Explore the full collection of hairstyles for service{" "}
                {serviceData.title.toLowerCase()}
              </p>
              ...
              {allStyles.length > 8 && (
                ...
                >
                  {showAllStyles ? (
                    ...
                    <>
                      View all {allStyles.length} hair model
                      <svg
              ...
              {displayedAllStyles.map((style, index) => (
                ...
                >
                  <div className="bg-white rounded-lg sm:rounded-xl overflow-hidden border border-pink-100 hover:border-pink-200 hover:shadow-xl transition-all duration-300">
                    ...
                    <div className="p-2.5 sm:p-3 md:p-4">
                      <h3 className="text-xs sm:text-sm md:text-base lg:text-lg font-medium text-gray-800 group-hover:text-pink-600 transition-colors mb-2 line-clamp-1">
                        {style.name}
                      </h3>
                      <p className="text-gray-500 text-xs sm:text-sm line-clamp-2 mb-2 sm:mb-3">
                        {style.description}
      ...
      {/* CTA */}
      <section className="py-12 sm:py-16 md:py-20 lg:py-24 bg-gradient-to-r from-pink-100 to-rose-100">
        <div className="max-w-5xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 text-center">
          <div className="bg-white rounded-xl sm:rounded-2xl md:rounded-3xl p-6 sm:p-8 md:p-12 border border-pink-200 shadow-xl">
            ...
            <p className="text-gray-600 mb-8 max-w-2xl mx-auto">
              Book an appointment today to experience high quality service{" "}
              {allStyles.length > 0
                ? `with ${allStyles.length} variety of hair styles`
                : ""}{" "}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="relative container max-h-full w-full h-full flex items-center justify-center p-2 sm:p-4">
            ...
            <button
              onClick={closeModal}
              className="absolute top-2 sm:top-4 right-2 sm:right-4 z-10 w-8 h-8 sm:w-10 sm:h-10 bg-white/90 backdrop-blur-sm border border-pink-200 rounded-full flex items-center justify-center text-pink-600 hover:bg-white transition-all shadow-lg"
            ...
            {serviceData.images.length > 1 && (
              <button
                ...
                className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-white/90 backdrop-blur-sm border border-pink-200 rounded-full flex items-center justify-center text-pink-600 hover:bg-white transition-all z-10 shadow-lg"
            ...
            <div className="relative max-w-full max-h-full">
              <img
                ...
                alt={`${serviceData.title} - Image ${modalImageIndex + 1}`}
            ...
            {serviceData.images.length > 1 && (
              <div className="absolute bottom-2 sm:bottom-4 left-1/2 -translate-x-1/2 flex space-x-1 sm:space-x-2 bg-white/90 backdrop-blur-sm rounded-lg p-1 sm:p-2 max-w-full overflow-x-auto shadow-lg">
                {serviceData.images.map((image, index) => (
                  <button
                    key={index}
                    onClick={() => setModalImageIndex(index)}
                    className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-md overflow-hidden transition-all flex-shrink-0 border-2 ${
